...
namespace Blazor.CMS.Editor.Infrastructure.Services
{
    public class AuditService : IAuditService
    {
        private readonly CMSEditorContext _context;
        ...
        {
            var trails = await _context.CMSEditorHistory.Where(a => a.UserId == userId).OrderByDescending(a => a.Id).Take(250).ToListAsync();
        ...
        public async Task<IResult<string>> ExportToExcelAsync(string userId, string searchString = "", bool searchInOldValues = false, bool searchInNewValues = false)
        {
            ...
            var trails = await _context.CMSEditorHistory
                .Specify(auditSpec)
                .OrderByDescending(a => a.DateTime)
                .ToListAsync();
            var data = await _excelService.ExportAsync(trails, sheetName: _localizer["CMS History"],
                mappers: new Dictionary<string, Func<CMSTracker, object>>
                {
